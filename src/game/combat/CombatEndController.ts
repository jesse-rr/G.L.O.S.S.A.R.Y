...
export class CombatEndController {
    ...
    show(result: CombatResult): void {
        ...
        if (result === 'DEFEAT') {
            ...
            new AudioManager(this.scene).playDefeat();
        } else if (result === 'VICTORY') {
            new AudioManager(this.scene).playVictory();
        }
    ...
    private continueFromCombat(result: CombatResult): void {
        ...
        const nm = NetworkManager.getInstance();
        if (nm.role !== 'offline') {
            nm.broadcast({ type: 'COMBAT_LEFT', originPeerId: nm.myPeerId });
        }

        if ((this.scene as any).audioManager) {
            (this.scene as any).audioManager.stopBossMusic(true);
        }
